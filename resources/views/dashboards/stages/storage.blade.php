        ...
        <div class="block-content block-content-narrow">
            <form action="/tech-cards/{{ $techCard->id }}/stage/{{ $stage->id}}" method="POST">
                ...
                <div class="form-group">
                    <label class="form-check-label">Библиотека</label>
                    <input class="form-control" type="number" id="material-text" name="library" placeholder="Please enter a library" value="{{ $data->library ?? '' }}">
                </div>

                <div class="form-group">
                    <label class="form-check-label">Автор</label>
                    <input class="form-control" type="number" id="material-text" name="author" placeholder="Please enter a author" value="{{ $data->author ?? '' }}">
                </div>
                ...
                {{-- @switch(Auth::user()->role->name)
                    ...
                    @case('executor')
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" id="responsible_input" name="responsible_id" placeholder="Please enter a number of pages" value="{{$data->employee->name}}" disabled>
                        </div>
                        @break
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="material-select" name="status" size="1" required>
                        ...
                        <option value="Получено" {{ !empty($data) && $data->status == "Получено" ? 'selected' : '' }}>Получено</option>
                        <option value="Передан автору" {{ !empty($data) && $data->status == "Передан автору" ? 'selected' : '' }}>Передан автору</option>
                    </select>
                </div>
